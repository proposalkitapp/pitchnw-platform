          ...
          {!selectedProposal ? (
            ...
            >
                  ...
                  {isFree && (
                    <div className="bg-amber-50 border border-amber-100 rounded-2xl px-5 py-3 flex items-center justify-between gap-6">
                      <p className="text-sm font-bold text-amber-700">
                        Free plan · {3 - Math.min(usedCount, 3)} left
                      </p>
                ...
                {isFree ? (
                  <>
                    ...
                    >
                      ...
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-70 mb-1">Elite Access</p>
                        <h3 className="text-xl font-black mb-4">Go Pro today</h3>
                        <Button className="w-full h-11 bg-white text-purple-600 hover:bg-slate-50 font-bold border-none shadow-none">Upgrade</Button>
                      </div>
                    </div>
                  </>
                ) : (
              ...
              <div className="bg-white border border-slate-100 rounded-[40px] p-10 flex flex-wrap items-center gap-4 shadow-sm">
                ...
                <Button className="h-14 px-8 rounded-2xl bg-[#0033ff] text-white font-bold hover:bg-[#002be6] shadow-lg shadow-blue-200 gap-2 active:scale-95 transition-all" disabled={isFree && usedCount >= 3} onClick={() => navigate('/generate')}>
                  <Plus className="h-4 w-4" /> + New Pitch
                </Button>
                {isPro && (
                  <>
                    <Button variant="outline" className="h-14 px-8 rounded-2xl border-slate-100 font-bold text-slate-600 hover:bg-slate-50" onClick={() => navigate('/proposals')}>Analyze</Button>
                    <Button variant="outline" className="h-14 px-8 rounded-2xl border-slate-100 font-bold text-slate-600 hover:bg-slate-50" onClick={() => navigate('/crm')}>CRM</Button>
                    <Button variant="outline" className="h-14 px-8 rounded-2xl border-slate-100 font-bold text-slate-600 hover:bg-slate-50" onClick={() => navigate('/coach')}>Coach</Button>
                  </>
                )}
                <Button variant="outline" className="h-14 px-8 rounded-2xl border-slate-100 font-bold text-slate-600 hover:bg-slate-50" onClick={() => navigate('/marketplace')}>Templates</Button>
              </div>
                  ...
                  <div className="bg-white rounded-[40px] border border-slate-100 overflow-hidden shadow-sm">
                    {proposals.length === 0 ? (
                      <div className="flex flex-col items-center justify-center p-20 text-center space-y-4">
                         ...
                         <Button className="rounded-xl h-12 px-6 bg-[#0033ff] text-white font-bold" onClick={() => navigate('/generate')}>Create First Pitch</Button>
                      </div>
                    ) : (
                        ...
                        <table className="w-full text-left">
                          <thead>
                            <tr className="border-b border-slate-50">
                              <th className="px-8 py-6 text-[10px] font-black uppercase text-slate-400">Pitch</th>
                          ...
                          <tbody>
                            {proposals.slice(0, 5).map((p) => (
                              <tr key={p.id} className="border-b border-slate-50 group hover:bg-slate-50/50 transition-colors">
                                ...
                                <td className="px-8 py-6 text-sm font-semibold text-slate-600">{p.client_name || "Prospect"}</td>
                                <td className="px-8 py-6">
                                ...
                                <td className="px-8 py-6">
                                  <div className="flex items-center justify-end gap-2">
                                    ...
                                    <ActionButton icon={Link2} onClick={() => copyProposalLink(p.public_slug)} tooltip="Link" />
                                    {isPro && (
                                      <>
                                        <ActionButton icon={BarChart2} onClick={() => navigate(`/proposals/${p.id}/analysis`)} tooltip="Analyze" />
                                        <ActionButton icon={Trash2} onClick={() => deleteProposal(p.id)} tooltip="Delete" color="text-red-400" />
                                      </>
                                    )}
                                  </div>
                                </td>
                ...
                <div className="space-y-10">
                  {isPro ? (
                    <>
                      {followups.length > 0 && (
                        <div className="bg-white border border-slate-100 border-l-amber-400 border-l-4 rounded-[32px] p-8 shadow-sm space-y-6">
                          <h4 className="flex items-center gap-2 font-black text-slate-900"><Clock className="h-5 w-5 text-amber-500" /> Follow-ups</h4>
                          {followups.map(p => (
                            <div key={p.id} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50">
                               ...
                               <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => navigate(`/proposals/${p.id}`)}><ArrowRight className="h-4 w-4" /></Button>
                            </div>
                          ))}
                        </div>
                      )}
                      ...
                      <div className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm space-y-6 cursor-pointer" onClick={() => navigate('/crm')}>
                        <h4 className="flex items-center gap-2 font-black text-slate-900"><BarChart2 className="h-5 w-5 text-[#0033ff]" /> Pipeline</h4>
                        <div className="h-5 w-full bg-slate-100 rounded-full overflow-hidden flex">
                          {pipelineSegments.map(s => <div key={s.label} className={s.color} style={{ width: `${s.width}%` }} />)}
                        </div>
                      </div>

                      {winRateData.length > 0 && (
                        <div className="bg-white border border-slate-100 rounded-[32px] p-8 shadow-sm space-y-6 h-[260px]">
                           <h4 className="flex items-center gap-2 font-black text-slate-900"><TrendingUp className="h-5 w-5 text-emerald-500" /> Trends</h4>
                           <ResponsiveContainer width="100%" height={150}>
                             <LineChart data={winRateData}>
                               <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F1F5F9" />
                               <XAxis dataKey="name" tick={{ fontSize: 10, fontWeight: 700 }} />
                               <RechartsTooltip />
                               <Line type="monotone" dataKey="rate" stroke="#0033ff" strokeWidth={3} dot={{ r: 4 }} />
                             </LineChart>
                           </ResponsiveContainer>
                        </div>
                      )}
                    ...
                    <div className="bg-slate-50 border border-slate-200 rounded-[40px] p-8 space-y-8">
                       <h4 className="font-display font-black text-xl text-slate-900">Unlock Pitchnw Pro</h4>
                       <div className="grid grid-cols-2 gap-4">
                          {[Brain, Target, Zap, Clock].map((Icon, i) => (
                            <div key={i} className="bg-white p-4 rounded-2xl opacity-60">
                               <Lock className="h-5 w-5 text-slate-300 mb-2" />
                               <div className="h-2 w-12 bg-slate-100 rounded" />
                            </div>
                          ))}
                       </div>
                       <Button className="w-full h-14 bg-purple-600 text-white rounded-2xl font-bold shadow-xl" onClick={() => navigate('/checkout')}>Upgrade now</Button>
                    </div>
                  )}
                </div>
              </div>
